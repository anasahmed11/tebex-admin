...
class Store extends Component {
    ...
    let products = this.state.products.map(product => 
        <Grid key={uuid()} md={4} sm={6} xs={12}>
            ...
            id={product.id}
            title={product.title}
            subtitle={product.salePrice? product.salePrice : product.price}
            subtitleOld={product.salePrice? product.price : false}
            currency={'جنيه'}
            img={product.img}
            flex />
        </Grid>
    ...
    return <Grid container className={classes.root}>
                ...
                <Grid md={9} sm={12}>
                    ...
                    <div className={classes.productsSection}>
                        {products.length? products: 'لا يوجد منتجات'}
                    </div>
                </Grid>
            </Grid>
    }
